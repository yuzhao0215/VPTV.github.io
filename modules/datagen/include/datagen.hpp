...
public:
	...
	void createStreaksImage(lpt::ImageFrame& frame);
	void createBlurStreamsImage(lpt::ImageFrame& frame);
	void createGaussianImage(lpt::ImageFrame& frame);
	cv::Mat image_type;
	double radius;
	int intensity;
...
public:
	...
	map< int, pair< vector<int>, vector<cv::Point3f> > > frames_midpoints;
	map< int, pair< vector<int>, vector<cv::Point3f> > > frames_endpoints;
	...
	void polyFitTraj(Trajectory& traj, int degree, double interval);
	void read3DTrajectoryFile(string filename, lpt::InputFormat format = lpt::PLAINTEXT, int timesOfTrajs=0);
	vector<lpt::Trajectory::Ptr> generateAdditionalTrajectories(int timesOfTrajs, vector<lpt::Trajectory::Ptr>& trajs);
