...
namespace lpt {
	...
	public:
		...
		{
			return std::make_shared<ParticleImage>(idnumber, x_pixel, y_pixel, radius, area, roundness);
		}
		...
		{
			return std::make_shared<ParticleImage>(idnumber, x_pixel, y_pixel, radius, intensity,
				max_intensity, mean_intensity, area, roundness, perimeter, xshift, yshift);
		}

		double x, y, radius, intensity, max_intensity, mean_intensity, area, roundness, perimeter;
		int x_max_shift, y_max_shift;  // these two are the deviation from the centroid to max pixel
		int id, match_count;
		bool is_4way_matched;

		ParticleImage() :id(-1), x(0), y(0), radius(0), intensity(0), is_4way_matched(false), match_count(0) {}

		ParticleImage(int idnumber, double x_pixel, double y_pixel, double radius, double intensity) :
			id(idnumber), x(x_pixel), y(y_pixel), radius(radius), intensity(intensity), is_4way_matched(false), match_count(0) {}

		ParticleImage(int idnumber, double x_pixel, double y_pixel, double radius, double a, double rou) :
			id(idnumber), x(x_pixel), y(y_pixel), radius(radius), area(a), roundness(rou), is_4way_matched(false), match_count(0) {}

		ParticleImage(int idnumber, double x_pixel, double y_pixel, double radius, 
			double intensity, double mi, double mei, 
			double a, double rou, double peri,
			int xshift, int yshift) :
			id(idnumber), x(x_pixel), y(y_pixel), radius(radius), 
			intensity(intensity), max_intensity(mi), mean_intensity(mei),
			x_max_shift(xshift), y_max_shift(yshift),
			area(a), roundness(rou), perimeter(peri), 
			is_4way_matched(false), match_count(0) {}
	};

	class ParticleStreakImage {
	public:
		typedef std::shared_ptr<ParticleStreakImage> Ptr;
		static inline ParticleStreakImage::Ptr create(
			int idnumber = -1,
			double radius = 0.0,
			double intensity = 0.0
		)
		{
			return std::make_shared<ParticleStreakImage>(idnumber, radius, intensity);
		}
		...
		vector<cv::Point2f> curvePoints2f;

		ParticleStreakImage() :id(-1), radius(0), intensity(0) {}
		ParticleStreakImage(int idnumber, double x_pixel, double y_pixel, double radius = 0, double intensity = 0) :
			id(idnumber), radius(radius), intensity(intensity) {}
	};
	...
	public:
		...
		vector<ParticleStreakImage::Ptr> streaks;
		vector<ParticleStreakImage::Ptr> streaks_floats;
	...
	public:
		...
		cv::Mat image_type;
		cv::Mat half_image_type;  // the image with (680 * 512) when using MJPEG mode
	...
	public:
		...
		Frame_() {}
		Frame_(int index): frame_index(index) {}
		...
		static inline std::shared_ptr<Frame_<Object_T>> create(int index = -1) {
			return std::make_shared<Frame_<Object_T>>(index);
		}
	};
	...
	// a different extrapolation function to return extrapolated points
	template<class Object_T>
	inline std::shared_ptr<Object_T> extrapolatePositionRet(lpt::Trajectory_<Object_T>& traj)
	{
		typedef typename Object_T::float_type float_type;
		...
		return std::make_shared<Object_T>(temp);
	}
