...
void Tracker::trackFrames(lpt::Frame3d& frame1, lpt::Frame3d& frame2 )
{
    ...
    for (list<lpt::Trajectory3d_Ptr>::iterator traj_iter = active_trajs.begin(); traj_iter != active_trajs.end(); ++traj_iter) {
        ...
        // If no candidate, remove trajectory from active list and add to trajectory list
        else {
			if ( (*traj_iter)->objects.size() > 4) {
	...
	// here extrapolated the trajectories again by iterate all active trajectories

	if (trajectories.size() > traj_count) {
		cout << std::scientific << std::setprecision(0);
		cout << "Tracked " << static_cast<double>(traj_count) << " trajectories" << endl;
		traj_count *= 10;
	}
}
