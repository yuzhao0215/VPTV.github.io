...
Chessboard::Chessboard(cv::Size board_size, double square_size) {
	...
	find_chessboard_flags = cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_FAST_CHECK | cv::CALIB_CB_NORMALIZE_IMAGE;
}
...
bool Chessboard::find(lpt::ImageFrame& frame) {
	...
	if (found) {
		cv::cornerSubPix(frame.image, image_points, cv::Size(11,11), cv::Size(-1,-1), cv::TermCriteria(cv::TermCriteria::EPS + cv::TermCriteria::MAX_ITER, 30, 0.1 ));
...
void Calibrator::addControls() {
	...
	cv::createButton("Get Int Data", callbackSetIntParamDataCollection, calibrator_void_ptr, cv::QT_CHECKBOX, 0 );
	cv::createButton("Clear Int Data", callbackClearIntParamData, calibrator_void_ptr, cv::QT_PUSH_BUTTON, 0 );
	cv::createButton("Calc Int Params", callbackRunIntParamCalibration, calibrator_void_ptr, cv::QT_PUSH_BUTTON, 0 );

	cv::createButton("Get Stereo Data", callbackSetStereoDataCollection, calibrator_void_ptr, cv::QT_CHECKBOX, 0 );
	cv::createButton("Clear Stereo Data", callbackClearStereoData, calibrator_void_ptr, cv::QT_PUSH_BUTTON, 0 );
	cv::createButton("Calc F Mats", callbackRunStereoCalibration, calibrator_void_ptr, cv::QT_PUSH_BUTTON, 0 );

	cv::createButton("Find Glob_Ref", callbackFindGlobalReference, calibrator_void_ptr, cv::QT_PUSH_BUTTON, 0 );
	...
	if (ok){
		...
		for (int i = 0; i < number_of_samples; ++i) {
			...
			double rms = cv::calibrateCamera(sample_3d_points, sample_2d_points,
							image_size, camera_matrix, dist_coeffs, rotation_vecs,
							translation_vecs, calib_flags | cv::CALIB_USE_INTRINSIC_GUESS | cv::CALIB_FIX_PRINCIPAL_POINT);
...
void Calibrator::findFundamentalMatrices() {
	...
	for (pair_it = camera_pairs.begin(); pair_it != camera_pairs.end(); ++pair_it) {
		...
		if (pointsA.size() > 8 && pointsB.size() > 8 ) {
			cv::Mat F = cv::findFundamentalMat(pointsA, pointsB, cv::FM_LMEDS, 1.0, 1.0-1E-6);
			F >> pair_it->F;
...
void Calibration::calcFundamentalMatrices(vector<Camera> &cameras, vector<CameraPair> &camera_pairs){
    		...
    		pair_it != camera_pairs.end(); ++pair_it)
    {
    	...
    	double rms = cv::stereoCalibrate(object_points, common_pointsA, common_pointsB,
    			...
    			image_size, R, T, E, F, calib_flags_stereo, cv::TermCriteria(cv::TermCriteria::MAX_ITER + cv::TermCriteria::EPS, 100, 1e-5)
    			);
