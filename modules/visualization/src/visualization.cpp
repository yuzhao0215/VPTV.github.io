...
namespace lpt {
	...
	void FiniteVolumeGrid::initialize()
	{
		...
		grid->AllocateScalars(VTK_INT, 1);  // MODIFIED FROM FOLLOWING TWO LINES
		...
		cellcentersfilter->SetInputData(grid);  // modified from following one line
		...
		plane_widget->GetPolyDataAlgorithm()->SetInputData(grid);
		...
		planeWidgetX->SetInputData(grid);
	...
	void FiniteVolumeGrid::updateGrid()
	{
		...
		//grid->GetProducerPort()->Modified();  // commented because of vtk6
	...
	void FiniteVolumeGrid::calcPressures()
	{
		...
		grid->GetCellData()->Modified();
		lookuptable->Modified();
		//   grid->GetProducerPort()->Modified();  // this line is commented because of vtk6
		//grid->GetProducerPort()->Modified();
		grid->Modified();
	...
	void TrajectoryHandler::addControls()
	{
		...
		cv::createButton("Clear TrajView", callbackClearTrajView, trajhandler_void_ptr, cv::QT_PUSH_BUTTON, 0);
		cv::createTrackbar("Update Stride", string(), &this->params.traj_update_stride, 60, 0, 0);
		cv::createButton("Reset Grid", callbackResetVolumeGrid, trajhandler_void_ptr, cv::QT_PUSH_BUTTON, 0);
		cv::createButton("Update Grid", callbackUpdateVolumeGrid, trajhandler_void_ptr, cv::QT_PUSH_BUTTON, 0);
		...
		cv::createButton("Show Cameras", callbackSetDisplayCameras, trajhandler_void_ptr, cv::QT_CHECKBOX, 1);
		cv::createButton("Save Data", callbackSavePlane, trajhandler_void_ptr, cv::QT_PUSH_BUTTON, 0);
	...
	void Visualizer::addControls()
	{
		...
		cv::createButton("Run Visualization", callbackSetVisualizationStatus, visualizer_void_ptr, cv::QT_CHECKBOX, 0);
		cv::createButton("Accumulate", callbackSetAccumulate, visualizer_void_ptr, cv::QT_CHECKBOX, 0);
		cv::createTrackbar("FrameStride", string(), &this->params.stride, 60, 0, 0);
		cv::createButton("Take Measurment", callbackTakeMeasurement, visualizer_void_ptr, cv::QT_PUSH_BUTTON, 0);
		handler->addControls();
	...
	StreamLines::StreamLines(vtkAlgorithmOutput *output_port)
	{
		...
		streamline->SetIntegrationStepUnit(0.5);
