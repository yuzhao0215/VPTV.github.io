...
bool Optitrack::initializeCameras() {
	...
	for( int i = 0; i < optitrack_cameras.size(); i++ ) {
		...
		// edited on 11/12/2019 to check grayscale exists
		...
		optitrack_cameras[camera_id]->SetFrameRate(30);   // Percentage for V120-SLIM Cameras
		optitrack_cameras[camera_id]->SetVideoType(Core::ObjectMode);
		optitrack_cameras[camera_id]->SetLateMJPEGDecompression(false);
		optitrack_cameras[camera_id]->SetLateMJPEGDecompression(true);
...
void Optitrack::addControls() {
	...
	cv::createButton("IR Filter", callbackSetIRFilter, cameras_void_ptr ,cv::QT_CHECKBOX, 1 );
	cv::createButton("Automatic Gain Control", callbackSetAGC, cameras_void_ptr, cv::QT_CHECKBOX, 1 );
	cv::createButton("Autoamtic Exposure Control", callbackSetAEC, cameras_void_ptr, cv::QT_CHECKBOX, 0 );
	...
	cv::createTrackbar("FrameRateMode", null , &init_framerate_mode, 3, callbackSetFrameRate, optitrack_void_ptr);
	...
	// this creates a track bar to select current modifying camera index
	cv::createTrackbar("Current Idx", null, &init_camera_idx, optitrack_cameras.size() - 1, callbackSetCurrentIdx, optitrack_void_ptr);
	cv::createTrackbar("Current video mode", null, &init_video_mode, 3, callbackSetCurrentVideoType, optitrack_void_ptr);
	cv::createTrackbar("Current exp time", null, &init_exposure, max_exposure-min_exposure, callbackSetCurrentExpTime, optitrack_void_ptr);
	cv::createTrackbar("Current threshold", null, &init_threshold, max_threshold - min_threshold, callbackSetCurThreshold, optitrack_void_ptr);
	cout<<"inside the add controls"<<endl;
}

//-------------OPTITRACK CALL BACK FUNCTIONS ------------------------------
void callbackSetCurrentIdx(int idx, void* data) {
	Optitrack* system = static_cast< Optitrack*> (data);
	system->current_camera_idx = idx;
	cout << "-------Current camera index is: " << system->current_camera_idx << " " << endl;
}
...
void callbackSetVideoType( int mode, void* data )
{
	...
	for (int id = 0; id < optitrack_cameras.size(); id++) { 
		if (new_mode == Core::MJPEGMode && optitrack_cameras[id]->IsMJPEGAvailable() == false)
			...
			/*
			if (new_mode == Core::MJPEGMode){
				optitrack_cameras[id]->SetMJPEGQuality(1);
			}
			*/
		}
...
void callbackSetCurThreshold(int value, void* data) {
	...
	optitrack_cameras[system->current_camera_idx]->SetThreshold(value);
}
...
void StreamingPipeline::initializeControlWindow() {
	...
	cv::createButton("Show Detected", callbackSetDetectionView, system_void_ptr , cv::QT_CHECKBOX, 0 );
...
void StreamingPipeline::runControlWindow() {
	...
	while( camera_system->areCamerasRunning() ) {
		...
		if( monitor_queue.try_pop( frame3d ) ) {
			...
			lpt::ImageFrameGroup& camera_frames = frame3d->camera_frames;//here each camera_frame is an image
			...
			if ( calibrator->isStereoDataCollecting() ) //what is stereo data?
				if ( count % calibrator->getFrameInterval() == 0) //???what is frame interval?
			...
			if ( calibrator->isSettingGlobalReference() ) {
				...
				if (ref_found)
					calibrator->setGlobalReference(false);//why false??
			}
...
//	boost::chrono::system_clock::time_point start, stop;
	...
	bool first = true;
	while( camera_system->areCamerasRunning() ) {
		vector<cv::Point3d> object_points;  // vector to store the reprojection 3d points
		...
		if ( this->showDetectionView() ) {
			detector->drawResult( imageproc_frames[index] );   // TODO Modify this later to see if the particle image is right
...
//	boost::chrono::system_clock::time_point start, stop;
	...
	while( camera_system->areCamerasRunning() ) {	
		std::pair<lpt::ImageFrameGroup, vector<lpt::Match::Ptr> > newpair;//what is match ptr???
		...
		if (this->visualizer->getTakeImageMeasurement() ) {
			this->visualizer->accumulateCentroidDetectionUncertainty( matches );//what is this?
		}
